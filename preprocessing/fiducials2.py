...
class FpCollection:
    ...
    def get_fiducials(self, s: PPG, ext_peaks=None):
        ...
        ext_peaks = ext_peaks.squeeze()
        onsets = self.align_onsets(onsets, ext_peaks, s)
        print(peaks.shape, ext_peaks.shape, len(onsets))
        dicroticnotch = self.get_dicrotic_notch(ext_peaks, onsets)

        vpg_fp = self.get_vpg_fiducials(onsets)
        apg_fp = self.get_apg_fiducials(onsets, ext_peaks)
        ...
        keys = ('on', 'sp', 'dn', 'dp')
        dummy = np.empty(len(ext_peaks)+1)
        dummy.fill(np.NaN)
        n = 0
        for temp_val in (onsets, ext_peaks, dicroticnotch, diastolicpeak):
            ppg_fp[keys[n]] = dummy
    ...
    def get_apg_fiducials(self, onsets: list, peaks: np.array):
        ...
        nan_v = np.empty(len(onsets) - 1)
        nan_v[:] = np.NaN
        a, b, c, d, e, f = copy.deepcopy(nan_v), copy.deepcopy(nan_v), copy.deepcopy(nan_v), copy.deepcopy(
            nan_v), copy.deepcopy(nan_v), copy.deepcopy(nan_v)
        for i in range(0, len(onsets) - 1):
            ...
            if correction.on[0]:
                try:
                    ...
                    if fiducials.on[i] != min_loc:
                        ...
                        fiducials.on[i] = np.argmax(self.jpg[int(fiducials.a[i]) - win_a:int(fiducials.a[i])]) + int(
                            fiducials.a[i]) - win_a
                except:
                    pass
            ...
            if correction.v[0] and correction.w[0]:
                if fiducials.v[i] > fiducials.e[i]:
                    try:
                        ...
                        fiducials.loc[i, 'w'] = [
                            find_peaks(self.vpg[int(fiducials.v[i]):int(fiducials.f[i])])[0][0] + fiducials.v[i]]
                    except:
                        pass
    ...
    def align_onsets(self, onsets, peaks, s):
        ...
        for i in range(len(peaks) - 1):
            ...
            if not possible_onsets:
                ...
                remaining_onsets = [onset for onset in remaining_onsets if
                                    (onset not in possible_onsets) and (onset > next_peak)]
        ...
        if remaining_onsets:
            ...
            filtered_onsets.append(int(len(s.ppg) - 1))

        return filtered_onsets


def get_dicrotic_notch(peaks: np.array, onsets: list):
    """
        Dicrotic Notch function estimate the location of dicrotic notch in between the systolic and diastolic peak
...
def get_vpg_fiducials(onsets: list):
    """Calculate first derivitive points u and v from the PPG' signal
...
def get_apg_fiducials(onsets: list, peaks: np.array):
    """Calculate Second derivitive points a, b, c, d, e, and f from the PPG" signal
...
def get_jpg_fiducials(onsets: list, apg_fp: pd.DataFrame):
    """Calculate third derivitive points p1 and p2 from the PPG'" signal
...
def get_diastolic_peak(onsets: list, dicroticnotch: list, e_point: pd.Series):
    """
        Dicrotic Notch function estimate the location of dicrotic notch in between the systolic and diastolic peak
